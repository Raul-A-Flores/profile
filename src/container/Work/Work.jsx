  ...
  return (
    <>
      ...
      <div className='app__work-filter'>
        {['UI/UX', 'Frontend', 'Backend', 'Fullstack', 'All'].map((item, index) => (
          <div key={index} 
          onClick={() => handleWorkFilter(item)}
          className={`app__work-filter-item app__flex ${active === item ? 'item-active' : ''}`}
          >{item}
          </div>
          ))} 
      ...
      <motion.div
        animate={animateCard}
        transition={{duration: 0.5, delayChilren: 0.5}}
